...
public static class VersionHelper
{
    ...
    public static async Task<bool> CheckForNewRelease(long? lastUpdateCheck, DateTimeOffset date)
    {
        if (lastUpdateCheck == null || lastUpdateCheck < date.AddDays(-7).ToUnixTimeSeconds())
        {
            if (CurrentVersion < await GetLatestGitHubReleaseVersion())
                MessageBox.Show(
                    $"A new version of Timmy Tools is available;{Environment.NewLine}https://github.com/timothywarner-org/TimmyTools/releases/latest",
                    "Update available",
                    MessageBoxButton.OK,
    ...
    private static async Task<Version?> GetLatestGitHubReleaseVersion()
    {
        ...
        client.DefaultRequestHeaders.UserAgent.Add(new ProductInfoHeaderValue("TimmyTools", CurrentVersion.ToString()));
        client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));

        try
        {
            HttpResponseMessage response = await client.GetAsync("https://api.github.com/repos/timothywarner-org/TimmyTools/releases/latest");
